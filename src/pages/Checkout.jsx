...
import { ArrowLeft, BadgePercent, CheckCircle2, CreditCard, Loader2, LockKeyhole, ShieldCheck, Smartphone, Landmark, Tag, XCircle } from "lucide-react";
...
const fallbackMethods = [
  ...
  { id: "jazzcash", name: "JazzCash", type: "manual", description: "Direct wallet payment with transaction ID verification.", enabled: false },
  { id: "easypaisa", name: "Easypaisa", type: "manual", description: "Direct wallet payment with transaction ID verification.", enabled: false },
  { id: "bank_transfer", name: "Bank Transfer", type: "manual", description: "Direct bank payment with transaction reference verification.", enabled: false },
];

const methodIcon = { payfast: CreditCard, jazzcash: Smartphone, easypaisa: Smartphone, bank_transfer: Landmark };
...
export default function Checkout() {
    ...
    const load = async () => {
      ...
      try {
        const [courseSnap, methodResponse] = await Promise.all([
          ...
          fetch("/api/payment-methods").then((response) => response.ok ? response.json() : null).catch(() => null),
        ]);
        if (!active) return;
        ...
        setMethods(available.length ? available : fallbackMethods.filter((item) => item.id === "payfast"));
  ...
  const selected = methods.find((item) => item.id === selectedMethod) || methods[0];
  ...
  const startPayment = async () => {
    ...
    if (selected?.type === "manual" && manualReference.trim().length < 4) {
      setMessage("Enter your transaction/reference number after making the transfer.");
      return;
    }
    ...
    try {
      ...
      const orderResponse = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ courseId, couponCode, paymentMethod: selectedMethod }),
      });
      ...
      if (Number(orderData.finalAmount) <= 0) {
        setMessage("Your discount covers the full course price. Course access is being unlocked.");
        navigate(`/payment/success?orderId=${encodeURIComponent(orderData.orderId)}`);
        return;
      }

      if (selected?.type === "manual") {
        const manualResponse = await fetch("/api/manual-payment-submit", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${idToken}` },
          body: JSON.stringify({ orderId: orderData.orderId, paymentMethod: selectedMethod, reference: manualReference, senderName }),
        });
        const manualData = await manualResponse.json();
        if (!manualResponse.ok) throw new Error(manualData.error || "Unable to submit payment reference");
        navigate(`/payment/success?orderId=${encodeURIComponent(orderData.orderId)}&manual=1`);
        return;
      }
        ...
        <div className="mt-7 grid gap-7 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="overflow-hidden rounded-3xl bg-slate-950 text-white shadow-2xl">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-7 grid gap-3 sm:grid-cols-2">{["Structured lessons", "Progress tracking", "Course completion", "Certificate eligibility"].map((item) => <div key={item} className="flex items-center gap-2 rounded-xl bg-white/5 p-3 text-sm font-semibold text-slate-200"><CheckCircle2 size={17} className="text-emerald-400" /> {item}</div>)}</div>
            </div>
          </section>

          <section className="rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-200 sm:p-8">
            <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><CreditCard size={22} /></div><div><h2 className="text-xl font-black text-slate-900">Order Summary</h2><p className="text-xs font-semibold text-slate-400">Secure PKR payment</p></div></div>
            ...
            <div className="mt-6"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-black text-slate-900">Choose payment method</h3><span className="text-[11px] font-bold text-slate-400">Multiple options</span></div><div className="mt-3 grid gap-2">{methods.map((method) => { const Icon = methodIcon[method.id] || CreditCard; const active = selectedMethod === method.id; return <button type="button" key={method.id} onClick={() => { setSelectedMethod(method.id); setMessage(""); }} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left transition ${active ? "border-blue-500 bg-blue-50 ring-2 ring-blue-500/10" : "border-slate-200 bg-white hover:border-blue-200"}`}><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${active ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}><Icon size={18} /></span><span className="min-w-0 flex-1"><span className="block text-sm font-black text-slate-900">{method.name}</span><span className="block text-xs leading-5 text-slate-500">{method.description}</span></span><span className={`h-4 w-4 rounded-full border-2 ${active ? "border-blue-600 bg-blue-600 ring-2 ring-white" : "border-slate-300"}`} /></button>; })}</div></div>

            {selected?.type === "manual" && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-black text-amber-900">Manual payment instructions</p><p className="mt-1 text-xs leading-5 text-amber-800">Transfer exactly {money(summary.finalAmount)} using the account details below, then enter the transaction/reference number. Admin will verify it before the course is unlocked.</p>{selected.accountName && <p className="mt-3 text-xs font-bold text-slate-800">Account Name: {selected.accountName}</p>}{selected.accountNumber && <p className="text-xs font-bold text-slate-800">Account Number: {selected.accountNumber}</p>}{selected.bankName && <p className="text-xs font-bold text-slate-800">Bank: {selected.bankName}</p>}{selected.iban && <p className="text-xs font-bold text-slate-800">IBAN: {selected.iban}</p>}<input value={senderName} onChange={(e) => setSenderName(e.target.value)} placeholder="Sender name (optional)" className="mt-3 w-full rounded-xl border border-amber-200 bg-white px-3 py-3 text-sm font-bold outline-none" /><input value={manualReference} onChange={(e) => setManualReference(e.target.value)} placeholder="Transaction / reference number" className="mt-2 w-full rounded-xl border border-amber-200 bg-white px-3 py-3 text-sm font-bold outline-none" /></div>}
            ...
            <div className="mt-5 grid gap-3 text-xs font-semibold text-slate-500 sm:grid-cols-2"><div className="flex items-center gap-2"><LockKeyhole size={16} className="text-emerald-600" /> Secure checkout</div><div className="flex items-center gap-2"><ShieldCheck size={16} className="text-blue-600" /> Verified payment</div></div>
            <p className="mt-5 text-center text-[11px] leading-5 text-slate-400">PayFast can present cards, bank accounts, mobile wallets and supported Raast options in its hosted checkout. Manual methods are verified by an admin before access is granted.</p>
          </section>
        </div>
      </div>
    </main>
