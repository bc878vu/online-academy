...
const PAYMENT_METHOD = {
  ...
  name: "PayFast",
  type: "gateway",
  description: "One secure checkout for JazzCash, Easypaisa, bank accounts, cards and supported Raast payments.",
  enabled: true,
  configured: true,
};
...
export default function Checkout() {
  ...
  const [selectedMethod] = useState("payfast");
  ...
  const startPayment = async () => {
    ...
    try {
      ...
      const orderResponse = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ courseId, couponCode, paymentMethod: "payfast" }),
      });
      ...
      if (Number(orderData.finalAmount) <= 0) throw new Error("This order cannot be processed online.");
      ...
      if (!paymentResponse.ok) throw new Error(paymentData.error || "Secure payment is temporarily unavailable. Please try again shortly.");
  ...
  return <main className="min-h-[calc(100vh-72px)] bg-slate-50 px-4 py-8 sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-7 grid gap-7 lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <section className="rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-200 sm:p-8">
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-blue-600" size={20} /><div><p className="text-sm font-black text-slate-900">{selected.name} secure payment</p><p className="mt-1 text-xs leading-5 text-slate-600">{selected.description}</p></div></div><div className="mt-4 flex flex-wrap gap-2 text-[11px] font-bold text-slate-600"><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-emerald-700">JazzCash</span><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-emerald-700">Easypaisa</span><span className="rounded-full bg-blue-50 px-2.5 py-1 text-blue-700">Bank</span><span className="rounded-full bg-blue-50 px-2.5 py-1 text-blue-700">Cards</span><span className="rounded-full bg-violet-50 px-2.5 py-1 text-violet-700">Raast</span></div></div>
        ...
        <button type="button" onClick={startPayment} disabled={busy || !selected.enabled} className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 text-sm font-black text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5 hover:from-blue-700 hover:to-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">{busy ? <Loader2 size={19} className="animate-spin" /> : <CreditCard size={19} />}{busy ? "Opening secure payment..." : `Pay ${money(summary.finalAmount)} securely`}</button>
        <div className="mt-5 grid gap-3 text-xs font-semibold text-slate-500 sm:grid-cols-2"><div className="flex items-center gap-2"><LockKeyhole size={16} className="text-emerald-600" /> Secure checkout</div><div className="flex items-center gap-2"><ShieldCheck size={16} className="text-blue-600" /> Verified before unlock</div></div>
        <p className="mt-5 text-center text-[11px] leading-5 text-slate-400">After successful payment, your course is unlocked automatically on this account.</p>
      </section>
    </div>
  </div></main>;
}
