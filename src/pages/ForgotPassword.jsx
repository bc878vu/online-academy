...
import { ArrowLeft, CheckCircle, Clock3, Loader2, Mail, ShieldCheck } from "lucide-react";
...
function ForgotPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Do not expose whether an account exists. This is safer and also works
      // correctly when Firebase Email Enumeration Protection is enabled.
      setSuccess(
        "If an account exists for this email, a password reset link has been sent. Check your inbox and spam/junk folder. The link is secure, expires automatically, and can only be used once."
      );
      setEmail("");
    } catch (firebaseError) {
      ...
      <div className="mx-auto flex w-full max-w-md items-center justify-center">
        <section className="w-full rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
          ...
          {success && (
            <div
              role="status"
              className="mt-6 flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium leading-6 text-emerald-700"
            >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                  className="w-full rounded-xl border border-slate-300 py-3 pl-11 pr-4 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-50"
                  required
                />
              </div>
            </div>
          ...
          <div className="mt-6 flex items-start gap-2 rounded-xl bg-slate-50 px-3.5 py-3 text-xs leading-5 text-slate-500 ring-1 ring-slate-100">
            ...
            <span>
              Reset links are controlled by Firebase Authentication. They expire
              automatically and become invalid after the password is changed.
            </span>
          </div>
