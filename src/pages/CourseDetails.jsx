...
const getCourseCacheKey = (id) => `${COURSE_CACHE_PREFIX}${id}`;
const progressStorageKey = (userId, courseId, lessonId) => `online_academy_progress_${userId}_${courseId}_${lessonId}`;

function getCachedCourse(id) {
  try {
    const parsed = JSON.parse(sessionStorage.getItem(getCourseCacheKey(id)) || "null");
...
function VideoLessonPlayer({ user, courseId, lesson, initialProgress, onSaved }) {
  ...
  const volumeRef = useRef(volume), mutedRef = useRef(muted);
  ...
  const persist = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (e?.code === "permission-denied") setError("Cloud attendance sync is blocked by Firebase rules. Your progress is safe on this device until the rules are published.");
      else setError("Cloud sync is temporarily unavailable. Your progress is safe on this device and will retry.");
  ...
  const handleSurfaceClick = useCallback(() => {
    ...
    if (!video) return;
    if (video.paused) video.play().catch(() => setError("Playback was blocked. Tap the video again to start it.")); else video.pause();
  }, [activity, playerReady, playing, showControls, source.type]);
  ...
  useEffect(() => {
    ...
    setDuration(stateRef.current.duration); setCurrentTime(stateRef.current.current); setEarnedSeconds(stateRef.current.earned); setPlaying(false); setError(""); setSyncState("idle"); setShowControls(false); setPlayerReady(false);
  ...
  }, [activity, flushActiveTime, persist, playing]);
  ...
  useEffect(() => {
    if (source.type !== "youtube") { try { ytPlayerRef.current?.destroy?.(); } catch {} ytPlayerRef.current = null; setPlayerReady(false); return undefined; }
    ...
    loadYouTubeAPI().then((YT) => {
      ...
      const player = new YT.Player(target, {
        width: "100%", height: "100%", videoId: source.videoId, host: "https://www.youtube-nocookie.com",
        playerVars: { autoplay: 0, controls: 0, rel: 0, modestbranding: 1, playsinline: 1, enablejsapi: 1, fs: 0, origin: window.location.origin, widget_referrer: window.location.href },
        events: {
          onReady: (event) => {
            if (cancelled) return;
            const d = Number(event.target.getDuration()) || 0; stateRef.current.duration = d; setDuration(d); setPlayerReady(true);
            const local = user ? readLocalProgress(user.uid, courseId, lesson.id) : null; const p = Number(initialProgress?.positionSeconds) || Number(local?.positionSeconds) || 0;
            if (p > 0 && p < d) event.target.seekTo(p, true);
            event.target.setVolume(volumeRef.current); if (mutedRef.current) event.target.mute();
          },
          ...
          onError: (event) => { const code = Number(event?.data); const text = code === 150 || code === 101 ? "This YouTube video does not allow embedding. Ask the admin to use an embeddable video URL." : code === 100 ? "This YouTube video was not found or is private." : "This lesson video could not be loaded. Check the video URL."; setError(text); setPlayerReady(false); },
        },
      });
      ytPlayerRef.current = player;
      timeoutId = window.setTimeout(() => { if (!cancelled && !ytPlayerRef.current) setError("Video is taking too long to load. Check the lesson video URL or YouTube embedding permission."); }, 12000);
    }).catch(() => setError("YouTube player could not be initialized. Please refresh the lesson."));
    return () => { cancelled = true; clearTimeout(timeoutId); try { ytPlayerRef.current?.destroy?.(); } catch {} ytPlayerRef.current = null; setPlayerReady(false); };
  ...
  const toggleMute = () => { const next = !muted; setMuted(next); if (source.type === "youtube") next ? ytPlayerRef.current?.mute?.() : ytPlayerRef.current?.unMute?.(); else if (videoRef.current) videoRef.current.muted = next; activity(); };
  ...
  useEffect(() => {
    const key = (event) => { if (["INPUT", "SELECT", "TEXTAREA"].includes(event.target?.tagName)) return; if (event.code === "Space") { event.preventDefault(); handleSurfaceClick(); } if (event.key === "ArrowLeft") { event.preventDefault(); seekBy(-10); } if (event.key === "ArrowRight") { event.preventDefault(); seekBy(10); } if (event.key.toLowerCase() === "m") { event.preventDefault(); toggleMute(); } if (event.key.toLowerCase() === "f") { event.preventDefault(); toggleFullscreen(); } };
  ...
  return <section ref={wrapRef} className={`overflow-hidden bg-slate-950 shadow-2xl ${fullscreen ? "flex h-screen flex-col rounded-none" : "rounded-2xl sm:rounded-3xl"}`} onMouseMove={activity} onTouchStart={activity}>
    <div className={`relative w-full bg-black ${fullscreen ? "min-h-0 flex-1" : "aspect-video min-h-[210px] sm:min-h-0"}`}>
      {source.type === "youtube" && <div ref={ytBoxRef} className="absolute inset-0 z-0 h-full w-full" aria-label="YouTube lesson player" />}
      ...
      {source.type !== "none" && source.type !== "invalid" && <div className={`absolute inset-x-0 bottom-0 z-30 bg-gradient-to-t from-black/95 via-black/60 to-transparent px-2 pb-2 pt-14 transition-all duration-300 sm:px-4 sm:pb-4 sm:pt-20 ${showControls ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-3 opacity-0"}`} onClick={(event) => event.stopPropagation()} onMouseMove={(event) => { event.stopPropagation(); activity(); }} onTouchStart={(event) => { event.stopPropagation(); activity(); }}>
        ...
        <div className="flex flex-col gap-2 text-white sm:flex-row sm:items-center sm:gap-2"><div className="flex min-w-0 items-center gap-1.5 sm:gap-2"><button type="button" onClick={handleSurfaceClick} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20" aria-label={playing ? "Pause" : "Play"}>{playing ? <Pause size={17} /> : <Play size={17} />}</button><button type="button" onClick={() => seekBy(-10)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20" aria-label="Back 10 seconds"><RotateCcw size={17} /></button><button type="button" onClick={() => seekBy(10)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20" aria-label="Forward 10 seconds"><RotateCw size={17} /></button><span className="ml-1 min-w-[72px] text-[11px] font-bold tabular-nums text-slate-200 sm:min-w-[84px] sm:text-xs">{formatTime(currentTime)} / {formatTime(duration)}</span><span className="hidden rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] font-black text-emerald-300 md:inline-flex">Active {percent}%</span></div><div className="flex items-center justify-end gap-1.5 sm:ml-auto"><button type="button" onClick={toggleMute} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20" aria-label={muted ? "Unmute" : "Mute"}>{muted || volume === 0 ? <VolumeX size={17} /> : <Volume2 size={17} />}</button><input aria-label="Volume" type="range" min="0" max="100" value={volume} onChange={(event) => setVolumeSafe(event.target.value)} className="hidden w-16 accent-blue-500 md:block lg:w-20" /><select value={speed} onChange={(event) => changeSpeed(event.target.value)} className="h-9 rounded-lg border border-white/10 bg-white/10 px-2 text-xs font-bold text-white outline-none"><option value="0.75" className="text-slate-900">0.75x</option><option value="1" className="text-slate-900">1x</option><option value="1.25" className="text-slate-900">1.25x</option><option value="1.5" className="text-slate-900">1.5x</option><option value="1.75" className="text-slate-900">1.75x</option><option value="2" className="text-slate-900">2x</option></select>{source.type === "html5" && <button type="button" onClick={togglePip} className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20 md:inline-flex" aria-label="Picture in picture"><PictureInPicture2 size={17} /></button>}<button type="button" onClick={toggleFullscreen} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 hover:bg-white/20" aria-label={fullscreen ? "Exit fullscreen" : "Fullscreen"}>{fullscreen ? <Minimize size={17} /> : <Maximize size={17} />}</button></div></div>
      </div>}
    </div>
...
export default function CourseDetails() {
  ...
  const requiredDefault = clampPercent(course?.attendance?.requiredWatchPercent, 25);
