...
const progressKey = (userId, courseId, lessonId) => `online_academy_progress_${userId}_${courseId}_${lessonId}`;

function readLocalProgress(userId, courseId, lessonId) {
  try { return JSON.parse(localStorage.getItem(progressKey(userId, courseId, lessonId)) || "null"); } catch { return null; }
}
function writeLocalProgress(userId, courseId, lessonId, data) {
  try { localStorage.setItem(progressKey(userId, courseId, lessonId), JSON.stringify({ ...data, localSavedAt: Date.now() })); } catch {}
}
...
function saveCourseCache(id, course) {
  try { sessionStorage.setItem(courseCacheKey(id), JSON.stringify({ timestamp: Date.now(), course })); } catch {}
}
function normalizeLessons(course) {
  const required = clampPercent(course?.attendance?.requiredWatchPercent, 25);
...
function VideoLessonPlayer({ user, courseId, lesson, initialProgress, onSaved }) {
        ...
        const player = new YT.Player(youtubeContainerRef.current, {
          ...
          events: {
            onReady: (event) => {
              ...
              setPlayerReady(true);
              const local = user ? readLocalProgress(user.uid, courseId, lesson.id) : null;
              const savedPosition = Number(initialProgress?.positionSeconds) || Number(local?.positionSeconds) || 0;
              if (savedPosition > 0 && savedPosition < d) event.target.seekTo(savedPosition, true);
  ...
  const setVolumeSafe = (value) => {
    ...
    setVolume(next);
    setMuted(next === 0);
    if (source.type === "youtube") ytPlayerRef.current?.setVolume?.(next);
    else if (videoRef.current) { videoRef.current.volume = next / 100; videoRef.current.muted = next === 0; }
  };
  const toggleMute = () => {
    const next = !muted;
    setMuted(next);
    if (source.type === "youtube") next ? ytPlayerRef.current?.mute?.() : ytPlayerRef.current?.unMute?.();
  ...
  const toggleFullscreen = async () => {
    try { if (document.fullscreenElement) await document.exitFullscreen(); else await wrapRef.current?.requestFullscreen?.(); }
  ...
  const togglePip = async () => {
    try {
      ...
      if (document.pictureInPictureElement) await document.exitPictureInPicture(); else await videoRef.current.requestPictureInPicture();
    } catch { setError("Picture-in-picture is available for supported direct video files."); }
    activity();
  };
...
export default function CourseDetails() {
  ...
  const loadCourse = useCallback(async () => {
    try {
      ...
      setError("");
    } catch (e) { setError(e?.code === "permission-denied" ? "You do not have permission to view this course." : e?.message || "Unable to load this course."); }
    ...
    const loadProgress = async () => {
      ...
      try {
        ...
        snap.docs.forEach((entry) => {
          ...
          map[data.lessonId] = { ...map[data.lessonId], ...data,
