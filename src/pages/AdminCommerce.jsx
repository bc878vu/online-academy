...
import { ArrowRight, BadgePercent, CheckCircle2, CreditCard, RefreshCw, Save, UserCog, Users, Megaphone, WalletCards } from "lucide-react";
...
export default function AdminCommerce() {
  ...
  useEffect(() => {
    if (view === "users" || view === "promotions" || view === "finance") {
      setLoading(false);
      return undefined;
    }
    load();
    ...
    <main className="min-h-[calc(100vh-72px)] bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
          ...
          <Link to="/admin/commerce?view=finance" className="inline-flex min-h-10 items-center gap-2 rounded-xl px-4 text-sm font-black text-slate-600 transition hover:bg-slate-50 hover:text-blue-700"><WalletCards size={16} /> Finance & Billing</Link>
          ...
          <Link to="/admin/commerce?view=promotions" className="inline-flex min-h-10 items-center gap-2 rounded-xl px-4 text-sm font-black text-slate-600 transition hover:bg-slate-50 hover:text-indigo-700"><Megaphone size={16} /> Offers & Promotions</Link>
        </div>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-black uppercase tracking-wider text-blue-600">Commerce</p><h1 className="mt-1 text-3xl font-black tracking-tight text-slate-950">Paid Courses</h1><p className="mt-2 text-sm text-slate-500">Set course prices and sale prices. Checkout validates the final price on the server.</p></div><div className="flex flex-wrap gap-2"><Link to="/admin/commerce?view=finance" className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-700"><WalletCards size={17} /> Finance & Billing <ArrowRight size={16} /></Link><Link to="/admin/commerce?view=users" className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-700"><UserCog size={17} /> Manage Users <ArrowRight size={16} /></Link><Link to="/admin/commerce?view=promotions" className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm font-bold text-indigo-700"><Megaphone size={17} /> Manage Offers <ArrowRight size={16} /></Link><button onClick={load} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700"><RefreshCw size={17} /> Refresh</button><Link to="/admin/discounts" className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold text-white"><BadgePercent size={17} /> Discounts <ArrowRight size={16} /></Link></div></div>
