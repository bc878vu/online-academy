...
import { Search, RefreshCw, Users, UserCog, X, CheckCircle2, Ban, MailCheck, MapPin, GraduationCap, Heart, Globe2, BriefcaseBusiness, Save, Eye, ChevronDown } from "lucide-react";
import { auth } from "../firebase";

const EMPTY = {
  displayName: "", email: "", photoUrl: "", emailVerified: false, disabled: false, username: "", phone: "",
  gender: "", dateOfBirth: "", maritalStatus: "", city: "", country: "Pakistan", address: "", education: "",
  currentStudy: "", institution: "", profession: "", occupation: "", bio: "", website: "", skills: [], languages: [], interests: [], socialLinks: {}
};
...
const initials = (u) => (u.displayName || u.email || "U")
  .split(/\s+/).filter(Boolean).slice(0, 2).map((x) => x[0]).join("").toUpperCase();

const countBy = (users, key) => {
  const map = new Map();
  users.forEach((u) => {
    const value = String(u[key] || "Not specified");
...
function Metric({ label, value, icon: Icon }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex items-center justify-between"><span className="text-sm font-bold text-slate-500">{label}</span><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><Icon size={18} /></span></div>
...
function BarChart({ title, icon: Icon, rows }) {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-4 space-y-3">{rows.slice(0, 6).map(([label, value]) => <div key={label}>
    ...
    </div>)}{rows.length === 0 && <p className="text-xs text-slate-400">No profile data yet.</p>}</div>
  </div>;
}

function FilterSelect({ value, onChange, label, options }) {
  return <label className="relative"><select value={value} onChange={(e) => onChange(e.target.value)} className="h-11 w-full appearance-none rounded-xl border border-slate-200 bg-white px-3 pr-8 text-xs font-black text-slate-700 outline-none focus:border-blue-400">
    <option value="all">All {label}</option>{options.map((x) => <option key={x} value={x}>{x}</option>)}
  </select><ChevronDown size={13} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-slate-400" /></label>;
}

function Avatar({ u }) {
  return u.photoUrl ? <img src={u.photoUrl} alt="" className="h-10 w-10 shrink-0 rounded-xl object-cover ring-1 ring-slate-200" /> : <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-xs font-black text-white">{initials(u)}</span>;
}

function TextField({ label, value, onChange, area = false, type = "text" }) {
  return <label className="block text-xs font-black text-slate-600">{label}{area ? <textarea rows={4} value={value || ""} onChange={(e) => onChange(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 p-3 text-sm font-semibold outline-none focus:border-blue-500" /> : <input type={type} value={value || ""} onChange={(e) => onChange(e.target.value)} className="mt-1.5 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-semibold outline-none focus:border-blue-500" />}</label>;
}
...
function ProfileModal({ user, onClose, onEdit, onToggle }) {
  const rows = [
    ...
    ["Date of birth", user.dateOfBirth], ["Age", user.age || ageOf(user.dateOfBirth)], ["Marital status", user.maritalStatus],
  ...
  return <div className="fixed inset-0 z-[90] overflow-y-auto bg-slate-950/60 p-3 backdrop-blur-sm sm:p-6"><div className="mx-auto max-w-4xl rounded-3xl bg-white shadow-2xl">
    <div className="flex items-center justify-between border-b p-5"><div className="flex items-center gap-3"><Avatar u={user}/><div><h2 className="text-xl font-black text-slate-950">{user.displayName || "Unnamed user"}</h2><p className="text-xs font-bold text-slate-400">Complete learner profile</p></div></div><button onClick={onClose} className="rounded-xl border p-2"><X size={19}/></button></div>
    <div className="p-5 sm:p-7"><div className="grid gap-4 sm:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-4"><p className="text-[10px] font-black uppercase text-blue-500">Profile completion</p><p className="mt-1 text-2xl font-black text-blue-800">{user.profileCompletion || 0}%</p></div><div className="rounded-2xl bg-emerald-50 p-4"><p className="text-[10px] font-black uppercase text-emerald-500">Courses</p><p className="mt-1 text-2xl font-black text-emerald-800">{user.paidCourses || 0}</p></div><div className="rounded-2xl bg-violet-50 p-4"><p className="text-[10px] font-black uppercase text-violet-500">Assessments</p><p className="mt-1 text-2xl font-black text-violet-800">{user.assessmentAttempts || 0}</p></div></div>
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5"><h3 className="font-black text-slate-900">About</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-600">{user.bio || "No bio provided yet."}</p></div>
      ...
      <div className="mt-6 flex flex-wrap gap-2"><button onClick={onEdit} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-black text-white"><UserCog size={17}/> Edit profile</button><button onClick={onToggle} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-black text-slate-700">{user.disabled ? <CheckCircle2 size={17}/> : <Ban size={17}/>} {user.disabled ? "Activate" : "Deactivate"}</button></div>
    </div></div></div>;
}

function EditModal({ form, update, onClose, onSave, saving }) {
  return <div className="fixed inset-0 z-[95] overflow-y-auto bg-slate-950/60 p-3 backdrop-blur-sm sm:p-6"><form onSubmit={onSave} className="mx-auto max-w-5xl rounded-3xl bg-white shadow-2xl">
    <div className="flex items-center justify-between border-b p-5"><div><h2 className="text-xl font-black">Edit User Profile</h2><p className="text-xs text-slate-400">Update all profile categories from admin.</p></div><button type="button" onClick={onClose} className="rounded-xl border p-2"><X size={19}/></button></div>
    <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <SelectField label="Gender" value={form.gender} onChange={(v) => update("gender", v)} options={["Male", "Female", "Other", "Prefer not to say"]}/><SelectField label="Marital status" value={form.maritalStatus} onChange={(v) => update("maritalStatus", v)} options={["Single", "Married", "Engaged", "Divorced", "Widowed", "Prefer not to say"]}/>
...
export default function AdminUsers() {
  ...
  const load = async (refresh = false) => {
    ...
    catch (e) { setToast(e.message || "Unable to load users"); }
  ...
  const options = useMemo(() => ({
    genders: countBy(users, "gender").map(([x]) => x), countries: countBy(users, "country").map(([x]) => x), cities: countBy(users, "city").map(([x]) => x), marital: countBy(users, "maritalStatus").map(([x]) => x)
  }), [users]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return users.filter((u) => (!q || [u.displayName, u.email, u.id, u.username, u.city, u.country, u.education, u.currentStudy, u.profession].some((v) => String(v || "").toLowerCase().includes(q))) &&
      (gender === "all" || u.gender === gender) && (country === "all" || u.country === country) && (city === "all" || u.city === city) && (marital === "all" || u.maritalStatus === marital) &&
      (status === "all" || (status === "active" && !u.disabled) || (status === "disabled" && u.disabled)));
  }, [users, query, gender, country, city, marital, status]);

  const stats = useMemo(() => ({
    total: filtered.length, active: filtered.filter((u) => !u.disabled).length, verified: filtered.filter((u) => u.emailVerified).length,
    complete: Math.round(filtered.reduce((sum, u) => sum + Number(u.profileCompletion || 0), 0) / Math.max(1, filtered.length))
  }), [filtered]);
  ...
  const save = async (e) => {
    e.preventDefault();
    try { setSaving(true); const payload = { userId: editing.id, ...form, skills: Array.isArray(form.skills) ? form.skills : [], languages: Array.isArray(form.languages) ? form.languages : [], interests: Array.isArray(form.interests) ? form.interests : [] }; const data = await request("update", payload); setUsers((items) => items.map((u) => u.id === editing.id ? { ...u, ...(data.user || payload) } : u)); setEditing(null); setToast("User profile updated successfully."); }
    catch (e2) { setToast(e2.message || "Unable to save profile"); }
    finally { setSaving(false); }
  };
  const toggle = async (u) => {
    try { await request("update", { userId: u.id, disabled: !u.disabled }); setUsers((items) => items.map((x) => x.id === u.id ? { ...x, disabled: !u.disabled } : x)); setToast(u.disabled ? "User activated." : "User deactivated."); }
    catch (e) { setToast(e.message || "Unable to update status"); }
  };
  ...
  return <div className="min-h-[calc(100vh-150px)] bg-slate-50 px-3 py-6 sm:px-5 lg:px-8 lg:py-9"><div className="mx-auto max-w-[1480px]">
    ...
    {selected && <ProfileModal user={selected} onClose={() => setSelected(null)} onEdit={() => openEdit(selected)} onToggle={() => toggle(selected)}/>} {editing && <EditModal form={form} update={update} onClose={() => setEditing(null)} onSave={save} saving={saving}/>} 
  </div></div>;
}
