...
import { Award, BookOpen, ChevronRight, FileText, Home, LayoutDashboard, LogOut, Menu, ShieldCheck, User, X, CreditCard, BadgePercent } from "lucide-react";
  ...
  return <>
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl">
      <div className="mx-auto max-w-[1480px] px-3 sm:px-5 lg:px-8">
        <nav className="flex min-h-[72px] items-center gap-3">
          <Link to="/" className="flex min-w-0 flex-1 items-center gap-3"><Brand className="h-11 w-11" /><span><span className="block text-[17px] font-black text-slate-950 sm:text-lg">Online Academy</span><span className="hidden text-[9px] font-bold tracking-[0.12em] text-slate-400 sm:block">LEARN. GROW. <span className="text-blue-600">SUCCEED.</span></span></span></Link>
          <div className="hidden items-center gap-1 rounded-2xl border border-slate-200 bg-slate-50 p-1 lg:flex">{links.map(([to, label, Icon]) => <Link key={to} to={to} className={`inline-flex h-10 items-center gap-2 rounded-xl px-3 text-xs font-extrabold ${active(to) ? "bg-white text-blue-700 shadow-sm" : "text-slate-600 hover:text-blue-700"}`}><Icon size={16} />{label}</Link>)}</div>
          <div className="ml-auto flex items-center gap-2">
            {user ? <Link to="/dashboard" className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-3.5 text-sm font-extrabold text-white shadow-md shadow-blue-600/20"><LayoutDashboard size={17} /><span className="hidden sm:inline">Dashboard</span></Link> : <Link to="/login" className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-extrabold text-white">Login <ChevronRight size={16} /></Link>}
            {user && <Link to="/profile" className="hidden h-11 w-11 items-center justify-center rounded-xl border border-slate-200 text-xs font-black text-slate-600 lg:flex">{initials}</Link>}
            <button onClick={() => setOpen((v) => !v)} className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 lg:hidden" aria-label="Menu">{open ? <X /> : <Menu />}</button>
          </div>
        </nav>
        <div className="flex gap-1 overflow-x-auto pb-2 lg:hidden">{links.slice(0, 3).map(([to, label, Icon]) => <Link key={to} to={to} className={`inline-flex shrink-0 items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-bold ${active(to) ? "border-blue-100 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600"}`}><Icon size={14} />{label}</Link>)}</div>
      </div>
    </header>
...
export default function App() {
  ...
  return <div className="flex min-h-screen flex-col bg-slate-50"><Navbar user={user} isAdmin={isAdmin} /><main className="min-w-0 flex-1"><Suspense fallback={<Loader text="Loading page..." />}><Routes>
    ...
    <Route path="/payment/success" element={<Protected user={user}><Checkout /></Protected>} />
    <Route path="/payment/failed" element={<Protected user={user}><Checkout /></Protected>} />
